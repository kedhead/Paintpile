...
export function FeedWidgetsRight() {
    ...
    useEffect(() => {
        async function loadWips() {
            ...
            try {
                // Fetch projects with status 'in-progress'
                ...
                console.error('Error loading wips:', err);
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <div className="space-y-6 sticky top-24">
                            ...
                            >
                                <div className="flex gap-3 mb-2">
                                    ...
                                    <div className="w-12 h-12 rounded bg-muted flex-shrink-0 overflow-hidden border border-border group-hover:border-orange-500/50 transition-colors">
                                        {/* In a real app we'd query the cover photo, but project object doesn't always have it on list view unless we sync it. 
                         For now, use a placeholder or check if we added photo support to project list items.*/}
                                ...
                                {/* Fake Progress Bar (based on something, or random for aesthetic if data missing, 
                    but better to use quantity painted/total if available. 
                    Let's assume painted/quantity) */}
            ...
            <div className="bg-gradient-to-br from-orange-950/20 to-background border border-orange-500/20 rounded-xl p-5 relative overflow-hidden group">
                ...
                <h4 className="text-lg font-display font-bold text-foreground mb-2 leading-tight">
                    #Golden_Aura
                </h4>

                <p className="text-sm text-muted-foreground mb-4">
                    Paint any miniature with glowing gold effects.
                </p>

                <div className="flex items-center gap-2 text-xs text-muted-foreground mb-4">
                    <span className="font-bold text-foreground">432</span> hobbyists participating
                </div>
