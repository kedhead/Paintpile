...
- Return ONLY the JSON object, no additional text`;
   ...
   * @returns Array of paint objects with brand and name
   */
  async expandPaintSet(
    description: string
  ): Promise<{ paints: { brand: string; name: string }[]; rawOutput: string }> {
    try {
      // Build the prompt
...
3. Return ONLY a JSON array of objects, where each object has "brand" and "name".
4. Do NOT output the set name itself in the array.

Example Output:
[
  {"brand": "Army Painter", "name": "Gravelord Grey"},
  {"brand": "Army Painter", "name": "Slaughter Red"},
  {"brand": "Citadel", "name": "Nuln Oil"}
]
...
JSON Output:`;
      ...
      const paints = JSON.parse(jsonMatch[0]) as { brand: string; name: string }[];
      return { paints, rawOutput };
    } catch (error: any) {
