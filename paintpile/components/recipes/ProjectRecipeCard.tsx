...
interface ProjectRecipeCardProps {
  recipe: PaintRecipe;
}

/**
 * LEGACY: This component displays old project-scoped recipes
 * Will be replaced after migration to unified recipe system
 */
export function ProjectRecipeCard({ recipe }: ProjectRecipeCardProps) {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {loading ? (
          <div className="text-sm text-muted-foreground">Loading paints...</div>
        ) : (
          <div className="space-y-3">
            {roleOrder.map(role => {
              ...
              return (
                <div key={role}>
                  <div className="text-xs font-semibold text-foreground uppercase mb-1">
                    {role}
                  </div>
                      ...
                      .map(entry => {
                        ...
                        return (
                          <div key={entry.paintId} className="flex items-center gap-2">
                            ...
                            {entry.ratio && (
                              <span className="text-xs text-muted-foreground">({entry.ratio})</span>
                            )}
                          </div>
                        );
                      })}
