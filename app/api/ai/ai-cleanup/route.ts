...
export const maxDuration = 60; // AI cleanup can take longer
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Check quota
    const estimatedCost = OPERATION_COSTS.aiCleanup;
    ...
    // Upload AI-cleaned image to Firebase Storage
    const storage = getAdminStorage();
    const bucket = storage.bucket();
    const storagePath = `users/${userId}/projects/${projectId}/photos/${photoId}/ai/${photoId}_ai_cleaned.png`;
    ...
    await trackUsage(userId, 'aiCleanup', actualCost);
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'AI cleanup completed successfully',
        data: {
          processedUrl,
    ...
    console.error('[AI] AI cleanup failed:', error);
    console.error('Error stack:', error.stack);

    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to perform AI cleanup',
